...
public class InformaçõesApostaLebre extends InformaçõesAposta{
    Scanner sc = new Scanner(System.in);

    private DadosLebre dados;
    private List<Lebre> lebres;
    private Login login;
    private int lebreVencedor;
    private boolean empate = false;

    public InformaçõesApostaLebre(Login login, DadosLebre dados) {
        super(0,0);
        this.login = login;
        this.dados = dados;
        lebres = dados.getLebres();
    }
    ...
    public void setLebreVencedor(int lebreVencedor) {
        this.lebreVencedor = lebreVencedor;
    }
    ...
    public void lerAnimal(){
        try {
            System.out.println();
            System.out.println("Escolha um lebre de 1 a 4 que deseja apostar:");
            animalEscolhido = sc.nextInt();
            while(animalEscolhido < 1 || animalEscolhido > 4){
                System.out.println("Escolha um lebre de 1 a 4 que deseja apostar:");
                animalEscolhido = sc.nextInt();
            }
        }catch (InputMismatchException e){
    ...
    public void ganhouPerdeu() {
        ...
        }else if(lebreVencedor == animalEscolhido) {
    ...
    public double recebePremio() {
        for (int i = 0; i < 4; i++) {
            if  (animalEscolhido == (i  + 1)) {
                System.out.println("SEU PREMIO FOI DE: " + valorAposta * lebres.get(i).getOdd());
                System.out.println();
                return (valorAposta * lebres.get(i).getOdd());
            }
        }
        return 0;
    }
